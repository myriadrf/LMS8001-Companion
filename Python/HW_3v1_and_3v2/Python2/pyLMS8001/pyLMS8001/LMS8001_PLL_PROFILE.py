...
class LMS8001_PLL_PROFILE(object):
    ...
    def R3_VAL(self):
        """
        Get the value of R3<3:0> calculated in Ohms
        """
        return self.calc_R3_VAL(self.R3)
    
    def calc_R3_VAL(self, R3_CODE):
        """
        Calculates the value of R3<3:0> in Ohms from Code Value
        """
        return (1e6 if (R3_CODE==0) else 14.9e3/R3_CODE)

    def calc_R3_CODE(self, R3_VAL):
        """
        Calculates the value of R3<3:0> code from Nom Value in Ohms
        """
        return min(max(int(round(14.9e3/R3_VAL), 1)), 15)
    ...
    def R2_VAL(self):
        """
        Get the value of R2<3:0> calculated in Ohms
        """
        return self.calc_R2_VAL(self.R2)
    
    def calc_R2_VAL(self, R2_CODE):
        """
        Calculates the value of R2<3:0> in Ohms from Code Value
        """
        return (1e6 if (R2_CODE==0) else 24.6e3/R2_CODE)

    def calc_R2_CODE(self, R2_VAL):
        """
        Calculates the value of R2<3:0> code from Nom Value in Ohms
        """
        return min(max(int(round(24.6e3/R2_VAL), 1)), 15)
    ...
    def C2_VAL(self):
        """
        Get the value of C2<3:0> in Farads
        """
        return self.calc_C2_VAL(self.C2)
	
    def calc_C2_VAL(self, C2_CODE):
        """
        Calculates the value of C1<3:0> in Farads from Code Value
        """
        return C2_CODE*10e-12+150e-12

    def calc_C2_CODE(self, C1_VAL):
        """
        Calculates the value of C2<3:0> code from Nom Value in Farads
        """
        return min(max(int(round((C2_VAL-150.0e-12)/10.0e-12)), 0), 15)
    ...
    def C1_VAL(self):
        """
        Get the value of C1<3:0> in Farads
        """
        return self.calc_C1_VAL(self.C1)

    def calc_C1_VAL(self, C1_CODE):
        """
        Calculates the value of C1<3:0> in Farads from Code Value
        """
        return C1_CODE*1.2e-12
        
    def calc_C1_CODE(self, C1_VAL):
        """
        Calculates the value of C1<3:0> code from Nom Value in Farads
        """
        return min(max(int(round(C1_VAL/1.2e-12), 0)), 15)
    ...
    @property 
    def VTUNE_VCT_VAL(self):
        """
        Get the value of VTUNE_VCT<1:0> calculated in Volts
        """
        return self.calcPLL_VTUNE_VCT_VAL(self.VTUNE_VCT)
    
    def calcPLL_VTUNE_VCT_VAL(self, VTUNE_VCT_CODE):
        """
        Calculates VTUNE_VCT Nom Value in Volts from Code value
        """
        if (VTUNE_VCT_CODE==3):
            VTUNE_VCT_VAL=900e-3
        elif (VTUNE_VCT_CODE==2):
            VTUNE_VCT_VAL=750e-3
        elif (VTUNE_VCT_CODE==1):
            VTUNE_VCT_VAL=600e-3
        else:
            VTUNE_VCT_VAL=300e-3

        return VTUNE_VCT_VAL

    def calc_VTUNE_VCT(self, VTUNE_VCT_VAL):
        """
        Calculates PLL-VTUNE_VCT Code Nom Value in Volts
        """
        vtune_vct_vals=[300e-3, 600e-3, 750e-3, 900e-3]
        vtune_err_vals=[]
        for vtune_vct in vtune_vct_vals:
            vtune_err_vals.append(abs(VTUNE_VCT_VAL-vtune_vct))

        return vtune_err_vals.index(min(vtune_err_vals))
    ...
    def C3_VAL(self):
        """
        Get the value of C3<3:0> calculated in Farads
        """
        return self.calc_C3_VAL(self.C3)
    
    def calc_C3_VAL(self, C3_CODE):
        """
        Calculates the value of C1<3:0> in Farads from Code Value
        """
        return C3_CODE*1.2e-12+5e-12
        
    def calc_C3_CODE(self, C3_VAL):
        """
        Calculates the value of C3<3:0> code from Nom Value in Farads
        """
        return min(max(int(round((C3_VAL-5.0e-12)/1.2e-12)), 0), 15)
    ...
    def DEL_VAL(self):
        """
        Get the value of DEL<1:0> calculated in seconds
        """
        return self.calc_DEL_VAL(self.DEL)
    
    def calc_DEL_VAL(self, PFD_DEL_CODE):
        """Calculates PFD-DEL Nom Value in Seconds from Code value"""
        return PFD_DEL_CODE*150e-12+400e-12
    
    def calc_PFD_DEL(self, PFD_DEL_VAL):
        """Calculates PFD-DEL Code from Nom Value in Seconds"""
        return min(max(int(round((PFD_DEL_VAL-400e-12)/150e-12), 0)), 3)
    ...
    def PULSE_VAL(self):
        """
        Get the value of PULSE<5:0> calculated in Amperes
        """
        PULSE = self.PULSE
        ICT_CP = self.ICT_CP
        return self.calc_PULSE_VAL(PULSE, ICT_CP)
    
    def calc_PULSE_VAL(self, PULSE, ICT_CP=16):
        """Calculates CP-PULSE Nom Value in Amperes from CODE value"""
        return PULSE*25*ICT_CP/16*1e-6
    
    def calc_PULSE_CODE(self, ICP_VAL, ICT_CP=16):
        """Calculates CP-PULSE Code from Nom Value in Amperes"""
        return min(max(int(round(ICP_VAL*16.0/ICT_CP/25.0e-6), 0)), 63)
    ...
    def OFS_VAL(self):
        """
        Get the value of OFS<5:0> calculated in Amperes
        """
        OFS = self.OFS
        ICT_CP = self.ICT_CP
        return self.calc_OFS_VAL(OFS, ICT_CP)
    
    
    def calc_OFS_VAL(self, OFS, ICT_CP=16):
        """Calculates CP-OFS Nom Value in Amperes from CODE value"""
        return OFS*25.0/4.0*ICT_CP/16*1e-6
    
    def calc_OFS_CODE(self, IOFS_VAL, ICT_CP=16):
        """Calculates CP-OFS Code from Nom Value in Amperes"""
        return min(max(int(round(IOFS_VAL*16.0/ICT_CP/6.25e-6), 0)), 63)
    ...
    def LD_VCT_VAL(self):
        """
        Get the value of LD_VCT<1:0> calculated in Volts
        """
        return self.calc_LD_VCT_VAL(self.LD_VCT)
    
    def calc_LD_VCT_VAL(self, LD_VCT_CODE):
        """
        Calculates LD_VCT Nom Value in Volts from Code value
        """
        return 600e-3+LD_VCT_CODE*100e-3
    
    def calc_LD_VCT(self, LD_VCT_VAL):
        """
        Calculates LD_VCT Code from Nom Value in Volts
        """
        return min(15, max(int(round((LD_VCT_VAL-600e-3)/100e-3)), 0))
    ...
    @property 
    def VDIV_SWVDD_VAL(self):
        """
        Get the value of VDIV_SWVDD<1:0> calculated in Volts
        """
        return self.calc_VDIV_SWVDD_VAL(self.VDIV_SWVDD)
    
    def calc_VDIV_SWVDD_VAL(self, VDIV_SWVDD_CODE):
        """
        Calculates VDIV_SWVDD Nom Value in Volts
        """
        return VDIV_SWVDD_CODE*200e-3+600e-3

    def calc_VDIV_SWVDD(self, VDIV_SWVDD_VAL):
        """
        Calculates VDIV_SWVDD Code from Nom Value in Volts
        """
        return min(15, max(int(round((VDIV_SWVDD_VAL-600e-3)/200e-3)), 0))
    ...
    def FF_MOD_VAL(self):
        """
        Get the value of FF_MOD<1:0> as integer division ratio 2**FF_MOD
        """
        return 2**(self.FF_MOD)
    ...
    def FRACMOD(self):
        """
        Calculate total fractional modulus of the feedback divider
        """
        FRACMODL = self.FRACMODL
        FRACMODH = self.FRACMODH
        return FRACMODH*(2**16)+FRACMODL
    ...
    def infoPFDCPConfig(self, printInfo=True):
        ...
        res += "|"+self.chip.fixLen("ON" if self.FLIP else "OFF", nChars_field-2) + "|" + self.chip.fixLen("%dps" %(400+150*self.DEL), nChars_field-1)+ "|" + self.chip.fixLen("%dmV" %(600+100*self.LD_VCT), nChars_field-1) + "|" + self.chip.fixLen("%.2fuA" %(self.ICT_CP*1.0/16*25), nChars_field-1) + "|" + self.chip.fixLen("%.2fuA" %(self.ICT_CP*1.0/16*25*self.PULSE), nChars_field-1) + "|" + self.chip.fixLen("%.2fuA" %(self.ICT_CP*1.0/16*25*self.OFS/4.0), nChars_field-1) +"|\n"
    ...
    def infoVCOConfig(self, printInfo=True):
        ...
        if (self.chip.MPW_YEAR==2024):
            if (VCO_SEL==1):
                VCO_SEL_TXT="4.65-6.40GHz"
            elif (VCO_SEL==2):
                VCO_SEL_TXT="6.35-7.95GHz"
            elif (VCO_SEL==3):
                VCO_SEL_TXT="7.90-9.70GHz"
            else:
                VCO_SEL_TXT="EXT-LO"
        else:
            if (VCO_SEL==1):
