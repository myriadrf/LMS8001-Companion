...
class LMS8001(object):
    ...
    def __init__(self, SPIwriteFn=None, SPIreadFn=None, MPW_YEAR=2024, verbose=0):
        self._fRef = 40e6
    ...
    def infoBias(self, printInfo=True):
        """
        Prinfo about Bias Block configuration.
        """
        regBiasConfig = self.getRegisterByName("BiasConfig")
        
        pd_calib_comp = regBiasConfig["PD_CALIB_COMP"]
        rp_calib_comp = regBiasConfig["RP_CALIB_COMP"]
        rp_calib_bias = regBiasConfig["RP_CALIB_BIAS<4:0>"]
        pd_frp_bias = regBiasConfig["PD_FRP_BIAS"]
        pd_f_bias = regBiasConfig["PD_F_BIAS"]
        pd_ptrp_bias = regBiasConfig["PD_PTRP_BIAS"]
        pd_pt_bias = regBiasConfig["PD_PT_BIAS"]
        pd_bias = regBiasConfig["PD_BIAS"]
        
        res = "-"*36+"\n"
        res += "| "+self.fixLen("BiasConfig (0x1400)", 33, center=True) +"|\n"
        res += "-"*36+"\n"
        res += "| "+self.fixLen("Signal Name", 20, center=True) +"|"
        res += self.fixLen("Value (dec)", 12, center=True)+"|\n"
        res += "-"*36+"\n"
        res += "| "+self.fixLen("PD_CALIB_COMP", 20, center=True) +"|"
        res += self.fixLen("%d" %(pd_calib_comp), 12, center=True)+"|\n"
        res += "| "+self.fixLen("RP_CALIB_COMP", 20, center=True) +"|"
        res += self.fixLen("%d" %(rp_calib_comp), 12, center=True)+"|\n"
        res += "| "+self.fixLen("RP_CALIB_BIAS<4:0>", 20, center=True) +"|"
        res += self.fixLen("%d" %(rp_calib_bias), 12, center=True)+"|\n"
        res += "| "+self.fixLen("PD_FRP_BIAS", 20, center=True) +"|"
        res += self.fixLen("%d" %(pd_frp_bias), 12, center=True)+"|\n"
        res += "| "+self.fixLen("PD_F_BIAS", 20, center=True) +"|"
        res += self.fixLen("%d" %(pd_f_bias), 12, center=True)+"|\n"
        res += "| "+self.fixLen("PD_PTRP_BIAS", 20, center=True) +"|"
        res += self.fixLen("%d" %(pd_ptrp_bias), 12, center=True)+"|\n"
        res += "| "+self.fixLen("PD_PT_BIAS", 20, center=True) +"|"
        res += self.fixLen("%d" %(pd_pt_bias), 12, center=True)+"|\n"
        res += "| "+self.fixLen("PD_BIAS", 20, center=True) +"|"
        res += self.fixLen("%d" %(pd_bias), 12, center=True)+"|\n"
        res += "-"*36+"\n"
    ...
    def calibrateBias(self):
        """
        Binary search algorith to find optimal value for RP_CALIB_BIAS<4:0>. Compensation of process variation effect on PD_F_BIAS and PD_PT_BIAS"
        """
        # pavlej update 22.11.2024.
		...
		# enable Bias Comparator for Rppoly calibration
        self.BIAS.PD_CALIB_COMP = 0
        
        # Enable Bias block if some of the bias currents are disabled. Enabled by default.
        if (self.BIAS.PD_FRP_BIAS or self.BIAS.PD_F_BIAS or self.BIAS.PD_PTRP_BIAS or self.BIAS.PD_PT_BIAS or self.BIAS.PD_BIAS):
            self.BIAS.PD_FRP_BIAS=0
            self.BIAS.PD_F_BIAS=0
            self.BIAS.PD_PTRP_BIAS=0
            self.BIAS.PD_PT_BIAS=0
            self.BIAS.PD_BIAS=0
        
      
        # binary search algorithm for optimal RP_CALIB_BIAS<4:0> value
        rp_calib_val=16
        for rp_calib_bit in range(4,-1, -1):
            rp_calib_val = rp_calib_val | (1<<rp_calib_bit)
            self.BIAS.RP_CALIB_BIAS = rp_calib_val
            if (self.BIAS.RP_CALIB_COMP==1):
                rp_calib_val=rp_calib_val | (1<<rp_calib_bit)
            else:
                rp_calib_val=rp_calib_val & (~(1<<rp_calib_bit))
        
        # disable Bias Comparator for Rppoly calibration
        self.BIAS.PD_CALIB_COMP = 1
