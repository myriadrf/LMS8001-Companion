...
class PLL_METHODS(object):
        ...
        if not (FIT_KVCO):
            if (self.chip.MPW_YEAR==2024):
                if (vco_sel==1):
                    ...
                    print 'Ext. LO selected in PLL_PROFILE.'
                    return None
            else:
            ...
            if (self.chip.MPW_YEAR==2024):
                if (vco_sel==1):
                    KVCO_avg=3.8492e+07 * (2.1005e-10 * CBANK**4 - 3.6267e-08 * CBANK**3 + 1.3634e-05 *CBANK**2 + 3.0541e-03 * CBANK**1 + 1.0011e+00 * CBANK**0)
                elif (vco_sel==2):
                    KVCO_avg=4.4058e+07 * (6.4250e-11 * CBANK**4 + 2.1831e-10 * CBANK**3 + 5.7102e-06 *CBANK**2 + 2.6286e-03 * CBANK**1 + 1.0011e+00 * CBANK**0)
                elif (vco_sel==3):
                    KVCO_avg=6.2508e+07 * (3.7053e-11 * CBANK**4 + 6.3153e-09 * CBANK**3 + 5.6397e-06 *CBANK**2 + 2.5792e-03 * CBANK**1 + 9.9837e-01 * CBANK**0)
                else:
                    print 'Ext. LO selected in PLL_PROFILE.'
                    return None
            else:
    ...
    def setLOFREQ(self, F_LO, XBUF_SLFBEN=1, IQ=False, IntN_Mode=False, CTUNE_METHOD='OPEN-LOOP', PROFILE=0, dbgMode=False):
        ...
        """
        if (IQ):
            if not (self.chip.PLL.F_VCO_MIN/16<=F_LO<=self.chip.PLL.F_VCO_MAX/2):
                self.chip.log("F_LO should be between %.3f MHz and %.3f GHz, when IQ=True. Failed to set LO Freq." %(self.chip.PLL.F_VCO_MIN/16/1.0e6, self.chip.PLL.F_VCO_MAX/2/1.0e9))
                return False
            DIV2IQ=1
        else:
            if not (self.chip.PLL.F_VCO_MIN/16<=F_LO<=self.chip.PLL.F_VCO_MAX):
                self.chip.log("F_LO should be between %.3f MHz and %.3f GHz. Failed to set LO Freq." %(self.chip.PLL.F_VCO_MIN/16/1.0e6, self.chip.PLL.F_VCO_MAX/2/1.0e9))
                return False
            if (self.chip.PLL.F_VCO_MIN/16<=F_LO<=self.chip.PLL.F_VCO_MIN/8):
                self.chip.log("F_LO values between %.3f MHz and %.3f MHz can only be generated when IQ=True. Failed to set LO Freq." %(self.chip.PLL.F_VCO_MIN/16/1.0e6, self.chip.PLL.F_VCO_MIN/8/1.0e6))
                return False
        ...
        while not (self.chip.PLL.F_VCO_MIN/2<=F_VCO<=self.chip.PLL.F_VCO_MAX):
            FFMOD+=1
    ...
    def configPLL(self, F_LO, IQ=False, autoConfXBUF=True, autoConfVREG=True, IntN_Mode=False, LoopBW=340.0e3, PM=55.0, FIT_KVCO=True, BWEF=1.0, FLOCK_N=200, SKIP_STEPS=[], CTUNE_METHOD='OPEN-LOOP', FLOCK_METHOD='SIMPLE', FLOCK_VCO_SPDUP=1, PROFILE=0, dbgMode=False):
        ...
        #print "autoConfVREG=", autoConfVREG
        #print 'tuning to ', F_LO
