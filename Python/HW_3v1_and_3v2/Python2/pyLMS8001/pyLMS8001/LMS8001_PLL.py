...
class LMS8001_PLL(object):
    def __init__(self, chip, fRef):
        ...
        if (self.chip.MPW_YEAR==2024):
            self.F_VCO_MAX=9.70e9
            self.F_VCO_MIN=4.70e9
            self.autoConfVREG=False
        else:
            self.F_VCO_MAX=9.11e9
            self.F_VCO_MIN=4.10e9
    ...
    def setLoopBW(self, LoopBW=340.0e3, PM=55.0):
        ...
        """
        # pavlej update 26.11.2024
        # adding autoConfXBUF=self.autoConfXBUF, autoConfVREG=self.autoConfVREG to prevent override of these attributes by configPLL method
        if (self.PLL_LOCK==1):
            status=self.PLLmethods.configPLL(F_LO=self.frequency, PROFILE=self.ACTIVE_PROFILE, IQ=False, autoConfXBUF=self.autoConfXBUF, autoConfVREG=self.autoConfVREG, LoopBW=LoopBW, PM=PM, SKIP_STEPS=[1,4])
            if (not status):
    ...
    @property 
    def VDIV_VCOREG_VAL(self):
        """
        Calculates the value of VDIV_VCOREG<7:0> in Volts
        """
        VDIV_VCOREG = self.VDIV_VCOREG
        BYP_VCOREG = self.BYP_VCOREG
        return 1.8 * (257.0-VDIV_VCOREG)/(265.0-VDIV_VCOREG) if (BYP_VCOREG==0) else 1.8
    
    @VDIV_VCOREG_VAL.setter
    def VDIV_VCOREG_VAL(self, value):
        """
        Sets the value of VDIV_VCOREG<7:0> closest to the desire 'value' in Volts
        """
        VDIV_VCOREG = int(round((1.8*257-265*value)/(1.8-value)))
        self._writeReg('PLL_VREG', 'VDIV_VCOREG<7:0>', int(max(0, min(VDIV_VCOREG, 255))))
        self._writeReg('PLL_VREG', 'BYP_VCOREG', 0)
